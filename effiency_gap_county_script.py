...
csv_filename = "condensed_results.csv"
...
with open(csv_filename, newline="") as csvfile:
    ...
    for row in spamreader:
        ...
        if row[4] != county:
            ...
            if net_votes == 0:
                net_votes = 1
                # print("!!!! 0 Net Votes for this County !!!!")
            ...
            if (
                net_wasted_dem_votes >= net_wasted_rep_votes
                and dem_seats_won <= rep_seats_won
            ) or (
                net_wasted_rep_votes >= net_wasted_dem_votes
                and rep_seats_won <= dem_seats_won
            ):
            ...
            county_write = [
                ...
                "",
                str(round(eff_gap, 4)),
                str(round(ideal_eff_gap, 4)),
            ]
            writer.writerow(county_write)
        ...
        if row[2] == "DEMOCRAT":
            ...
            current_row[1] = str(row[3])
            dem_votes += int(row[3])
        elif row[2] == "REPUBLICAN":
            # print("Rep Votes: " + str(row[3]))
            current_row[2] = str(row[3])
            rep_votes += int(row[3])
            ...
            current_row[3] = str(total_votes)
            # print("Votes needed for majority: " + str(votes_for_majority))
            current_row[4] = str(votes_for_majority)
            ...
            current_row[5] = str(dem_wasted_votes)
            # print("Rep wasted votes: " + str(rep_wasted_votes))
            current_row[6] = str(rep_wasted_votes)
